...
async function run() {
  ...
  for(let h = rootHeight; rootHeight - h < 512; h--) {
    ...
    if(root.miner.slice(0,4)=='0x32' && !check0x32) {
      check0x32 = true;
      console.log(`根链高度${Number(root.height)}, 当前难度${('00'+(root.difficulty/10e11).toFixed(7)).slice(-12)}T, 0x32地址出块数${('000'+Number(root.poswMinedBlocks)).slice(-3)}/512, 出块比例${('000'+(root.poswMinedBlocks*100/512).toFixed(2)).slice(-5)}%, 算力${('00000'+(root.difficulty/60/10000/512*root.poswMinedBlocks/1000).toFixed(2)).slice(-10)}K`);
    } else if(root.miner.slice(0,4)=='0xfc' && !check0xfc) {
      check0xfc = true;
      console.log(`根链高度${Number(root.height)}, 当前难度${('00'+(root.difficulty/10e11).toFixed(7)).slice(-12)}T, 0xfc地址出块数${('000'+Number(root.poswMinedBlocks)).slice(-3)}/512, 出块比例${('000'+(root.poswMinedBlocks*100/512).toFixed(2)).slice(-5)}%, 算力${('00000'+(root.difficulty/60/10000/512*root.poswMinedBlocks/1000).toFixed(2)).slice(-10)}K`);
    } else if(root.miner.slice(0,4)=='0x2b' && !check0x2b) {
      check0x2b = true;
      console.log(`根链高度${Number(root.height)}, 当前难度${('00'+(root.difficulty/10e11).toFixed(7)).slice(-12)}T, 0x2b地址出块数${('000'+Number(root.poswMinedBlocks)).slice(-3)}/512, 出块比例${('000'+(root.poswMinedBlocks*100/512).toFixed(2)).slice(-5)}%, 算力${('00000'+(root.difficulty/60/1000/512*root.poswMinedBlocks/1000).toFixed(2)).slice(-10)}K`);
    }
